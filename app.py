...
def render_boarding_pass(data):
    ...
    html_template = f"""
<div id="ticket" style="width:380px; padding:20px; font-family: 'Courier New', monospace; background:#fff; color:#000; font-size:12px; line-height:1.4; position: relative; margin: 0 auto;">

    <div style="display:flex;justify-content:space-between;align-items:center;">
        <div style="width:80px;">
            <img src="data:image/png;base64,{logo_base64}" style="width:100%; filter: grayscale(100%); -webkit-filter: grayscale(100%); display:block;">
        </div>
        <div style="text-align:center; line-height:1.2;">
            <b style="font-size:16px; font-family: sans-serif;">BOARDING PASS</b><br>
            <span style="font-size:13px; font-family: sans-serif;">Untuk Pengguna Jasa</span>
        </div>
        <div style="font-size:36px; font-weight:900; font-family: sans-serif;">B</div>
    </div>
    ...
    <div style="display:flex; justify-content:space-between; text-align:center;">
        <div style="width:35%; text-align:center;">
            <div style="font-size:11px;">ASAL</div>
            <b style="font-size:16px; font-family: sans-serif;">{data['Asal']}</b>
        </div>
        <div style="width:30%; font-size:24px; display:flex; align-items:center; justify-content:center;">
            &#9973; 
        </div>
        <div style="width:35%; text-align:center;">
            <div style="font-size:11px;">TUJUAN</div>
            <b style="font-size:16px; font-family: sans-serif;">{data['Tujuan']}</b>
        </div>
    </div>

    <div style="border: 1.5px solid #000; border-radius: 15px; padding: 5px 15px; margin-top: 10px; display: flex; justify-content: space-between; text-align: center; font-family: sans-serif; align-items: center; position: relative;">
        <div style="position: absolute; left: -8px; top: 8px; background: #fff; font-size: 16px;">&#9973;</div>
        
        <div>
            <div style="font-size: 9px;">&#9432; Kelas Layanan</div>
            <b style="font-size: 14px;">{data['Kelas Layanan']}</b>
        </div>
        <div>
            <div style="font-size: 9px;">Golongan</div>
            <b style="font-size: 14px;">{data['Golongan']}</b>
        </div>
        <div>
            <div style="font-size: 9px;">Total PNP</div>
            <b style="font-size: 14px;">{data['Total PNP']}</b>
        </div>
        <div>
            <div style="font-size: 9px;">Dermaga</div>
            <b style="font-size: 14px;">{data['Dermaga']}</b>
        </div>
        
        <div style="position: absolute; right: -8px; top: 8px; background: #fff; font-size: 16px;">&#9881;</div>
    </div>
    ...
    <div style="position: relative;">
        <div style="position: absolute; top: 15px; left: 0; right: 0; text-align: center; font-size: 55px; font-weight: bold; color: rgba(150, 150, 150, 0.3); z-index: 0; font-family: sans-serif; letter-spacing: 2px;">
            {data['Kelas Layanan']}
        </div>
        <table style="width: 100%; font-size: 13px; line-height: 1.6; position: relative; z-index: 1;">
            <tr><td style="width: 38%;">WAKTU CHECK-IN</td><td style="width: 2%;">:</td><td>{data['Waktu Check-In']}</td></tr>
            <tr><td>NO. TIKET</td><td>:</td><td>{data['No Tiket']}</td></tr>
            <tr><td>NAMA</td><td>:</td><td>{data['Nama']}</td></tr>
            <tr><td>NO. POLISI</td><td>:</td><td>{data['No Polisi']}</td></tr>
...
<button onclick="downloadImage()" style="margin: 0 auto; display: block; padding: 8px 16px; cursor: pointer;">📥 Download PNG</button>
...
"""
    st.components.v1.html(html_template, height=760)
